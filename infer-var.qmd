...
We use [`chi2`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.chi2.html) family in `scipy.stats` to get the $\chi^2$ critical values. The probability $p$ and the degrees of freedom `df` need to be specified. You should be able to understand the rest part of the code. Enjoy it.
...
We use [`f.ppf()`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.f.html) and `f.isf()` in `scipy.stats` to find the $F$ critical values. 
...
import pandas as pd
# Data
y1 = [4.2, 2.9, 0.2, 25.7, 6.3, 7.2, 2.3, 9.9, 5.3, 6.5]
y2 = [0.2, 11.3, 0.3, 17.1, 51, 10.1, 0.3, 0.6, 7.9, 7.2]
y3 = [7.2, 6.4, 9.9, 3.5, 10.6, 10.8, 10.6, 8.4, 6.0, 11.9]

# Creating a DataFrame
data_additive = pd.DataFrame({
    "mpg_increase": y1 + y2 + y3,
    "additive": pd.Categorical([1] * 10 + [2] * 10 + [3] * 10)
})
```
...
data_additive.boxplot(column="mpg_increase", by="additive", vert=False, grid=False)
plt.title("Boxplots for three additives")
plt.suptitle("")
plt.xlabel("mpg_increase")
plt.ylabel("Additive")
plt.show()
```
