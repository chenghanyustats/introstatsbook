...
In Python, to randomly sample an element of a vector, we can use the function [`np.random.choice()`](https://numpy.org/doc/stable/reference/random/generated/numpy.random.choice.html). For example, the following code shows how to randomly draw a ball from that bag:
...
This time we get a blue ball. To do a Monte Carlo simulation, we repeatedly draw one ball from the bag using a [for loop within a list](https://docs.python.org/3/tutorial/datastructures.html#list-comprehensions), so that the every draw is saved in a list.
...
To ensure the results are exactly the same every time you do sampling, in R we set the random seed to a specific number using the function [`np.random.seed()`](https://numpy.org/doc/stable/reference/random/generated/numpy.random.seed.html).
