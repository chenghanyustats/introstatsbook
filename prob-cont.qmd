...
- <span style="color:red"> Step 4: Find the area using `norm.cdf()` </span>
...
By default, `norm.cdf()` uses the standard normal distribution assuming `loc=0` and `scale=1`. So if we use z-scores to compute probabilities, we don't need to specify the value of mean and standard deviation, and our code is shorter:
...
## Python
  ...
  + [**`norm.ppf(p, loc, scale)`**](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.norm.html#scipy.stats.norm) to get a value of $X$, $x$, such that $P(X \le x) = p$ 
  
  + **`norm.isf(p, loc, scale)`** to get $x$ such that $P(X \ge x) = p$. `isf` is short for **i**nverse **s**urvival **f**unction.
...
- <span style="color:red"> Step 3: Find $z$-score s.t. $P(Z < z^*) = 0.95$ using `norm.ppf()`</span>: 
...
`r emoji('point_right')` To find a probability, if you are a beginner, it is always good to **draw and label the normal curve and shade the area of interest**. Below is a summary of how we can use `norm.cdf()` and `norm.sf()` to compute various kinds of probabilities.
...
To generate a QQ-plot for checking normality in R, we can use [`stats.probplot()`](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.probplot.html), where the first argument in the functions is the sample data we would like to check. @fig-qqplot-py shows QQ plots at the bottom for normal and right-skewed data samples. Since the data `normal_sample` actually come from a normal distribution, its histogram looks like normal, and its QQ plot look like a perfect straight line. On the other hand, on the right hand side we have a right skewed data set `right_skewed_sample`. Clearly, its QQ plot is a upward curve, and definitely not linear, indicating that the sample data are not normally distributed.
...
# Set up 2x2 plotting grid and adjust margins
fig, axs = plt.subplots(2, 2, figsize=(10, 8))
plt.subplots_adjust(hspace=0.5, wspace=0.3)

# Generate samples
normal_sample = np.random.normal(0, 1, 1000)
right_skewed_sample = np.random.gamma(2, 1/2, 1000)

# First plot: Histogram of normal data
_junk = axs[0, 0].hist(normal_sample, bins=20, color='blue', edgecolor='white');
axs[0, 0].set_title("Normal data")
axs[0, 0].set_xlabel("Value")
axs[0, 0].set_ylabel("Frequency")

# Second plot: QQ plot for normal data
_junk = stats.probplot(normal_sample, dist="norm", plot=axs[1, 0])
axs[1, 0].set_title("Normal data (QQ Plot)")

# Third plot: Histogram of right-skewed data
_junk = axs[0, 1].hist(right_skewed_sample, bins=20, color='magenta', edgecolor='white');
axs[0, 1].set_title("Right-skewed data")
axs[0, 1].set_xlabel("Value")
axs[0, 1].set_ylabel("Frequency")

# Fourth plot: QQ plot for right-skewed data
_junk = stats.probplot(right_skewed_sample, dist="norm", plot=axs[1, 1])
axs[1, 1].get_lines()[0].set_markerfacecolor('magenta')
axs[1, 1].set_title("Right-skewed data (QQ Plot)")
...
:::{.callout-note}
The code involves doing [subplots](https://matplotlib.org/stable/gallery/subplots_axes_and_figures/subplots_demo.html) in one single figure, and it makes the code a bit complex and long. No need to worry that much. 
:::
...
{{< include prob-binom-normal-ojs.qmd >}}
...
```{python}
#| echo: true
#| label: normal_approximation_to_binommial-py
