# Simple Linear Regression {#sec-model-reg}
...
In Python, we use the function [`corr()`](https://pandas.pydata.org/docs/reference/api/pandas.Series.corr.html#pandas.Series.corr) to calculate the correlation coefficient $r$ for two pd.Series. The syntax is like `first_pd.Series.corr(second_pd.Series)`.

Here we use the built-in `mtcars` dataset, and calculate the correlation between the high miles per gallon (`mpg`) and car weight (`wt`). The coefficient is $r =$ ```r round(cor(x = mtcars$wt, y = mtcars$mpg), 2)```. Usually, when $|r| > 0.7$, the linear relationship is considered strong, and $|r| < 0.3$ for weak linear relationship.
...
To fit a linear regression, we use the `ols()` function. As we learned in ANOVA @sec-model-anova, we write the formula `'hwy ~ displ'`, and specify the data set `mpg`. We save the fitted result in the object `reg_fit`. 
...
To predict either the mean response or a new observation, we use the method [`get_prediction()`](https://www.statsmodels.org/dev/generated/statsmodels.regression.linear_model.OLSResults.get_prediction.html). The first argument `exog` is the values for which you want to predict. `exog` means exogenous variables. Be careful that it has to be array_like type. 

If we want to get the confidence interval for the mean response, `predict.conf_int()` provides that. If instead we want the prediction interval for a new response, we need to check the `predict.summary_frame()`. The prediction interval is saved in 'obs_ci_lower' and 'obs_ci_upper'.
...
We can use bootstrap to estimate the regression coefficients with uncertainty quantification without assuming or relying on normal distribution. Instead of doing distribution-based inference, we are doing simulation-based inference. Please go to @sec-infer-bt to review the idea of bootstrapping.

In simple linear regression, we perform bootstrap inference step by step as follows.

1. *Bootstrap new samples $\{x^b_j, y^b_j\}_{j=1}^n$ from the original sample $\{x_i, y_i\}_{i=1}^n$.* Note that the bootstrap sample has the same size of the original sample.

2. *For each bootstrapped sample, we fit a simple linear regression model to it, and estimate the corresponding slope.* If we have $B$ bootstrapped samples, we then have $B$ different bootstrapped estimates of slope. Same idea for the intercept.

3. *Use the distribution of the bootstrapped slopes to construct a confidence interval.* We plot the histogram of those $B$ slopes (and/or intercepts). Use it as the distribution of the slope. Then we can for example construct a 95\% confidence interval for the slope by finding the 2.5\% and 97.5\% percentiles of the distribution.
...
#| out.width: 100%
#| message: false 
p_df_obs <- ggplot(mpg, aes(x = displ, y = hwy)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = FALSE, color = 2, linewidth = 2) +
  labs(
    x = "Displacement (litres)",
    y = "Highway MPG",
    title = "Original Highway MPG vs. Engine Displacement"
  ) +
...
#| out.width: "100%"
#| message: false 
ggplot(df_boot_samples_5, aes(x = displ, y = hwy, group = replicate)) +
  geom_line(stat = "smooth", method = "lm", se = FALSE, alpha = 0.5,
            linewidth = 1.5, color = 4) +
  labs(
    x = "Displacement (litres)",
    y = "Highway MPG",
...
#| out.width: "100%"
#| message: false 
p_df_boot_samples_100 <- ggplot(df_boot_samples_100, aes(x = displ, y = hwy, group = replicate)) +
...
```{r}
df_100_b0 <- df_boot_samples_100 |> fit() |> filter(term == "intercept")
df_100_b0_mean <- mean(pull(df_100_b0[, 3]))

df_100_b1 <- df_boot_samples_100 |> fit() |> filter(term == "displ")
df_100_b1_mean <- mean(pull(df_100_b1[, 3]))
```
...
p_df_boot_samples_100 +
  geom_abline(intercept = intercept, slope = slope, color = 2, 
              linewidth = 2, alpha = 0.5) +
  geom_abline(intercept = df_100_b0_mean, 
              slope = df_100_b1_mean, 
              color = "darkblue", alpha = 0.5,
              linewidth = 2)+
  labs(title = "Fitted line and Bootstrapped Mean Fitted Line")
```
:::
...
df_boot_samples_100_fit <- df_boot_samples_100 |> fit()

df_boot_samples_100_hist <- ggplot(df_boot_samples_100_fit %>% filter(term == "displ"), aes(x = estimate)) +
  geom_histogram(binwidth = 0.05, color = "white") +
  geom_vline(xintercept = slope, color = 4, linewidth = 2) +
  labs(x = "Slope", y = "Count",
       title = "Distribution of Slope and 95% CI")
...
As discussed in @sec-infer-bt, we can use the `boot` package and its function `boot()` to do bootstrappingin R.
...
We can get the confidence intervals for intercept and slope using `boot.ci()` as we do in @sec-infer-bt.
...
Note that `trans` has so many different categories or "values" because of the parentheses terms. We can remove them so that the `trans` variable only has two categories: `auto` and `manual`, where `auto` means automatic transmission, and `manual` means manual transmission. The cleaned data set is called `mpg_trans`.
...
```{python}
mpg_trans = mpg.copy()
mpg_trans['trans'] = np.where(mpg_trans['trans'].str.contains('auto'), 'auto', 'manual')
```
...
The idea is that when we fit a regression model with a categorical variable, we are learning how the (mean) response value changes when the category changes from the baseline level to another level. Notice that in the regression output, the slope coefficient label is `transmanual`. It is the concatenation of the categorical variable's name `trans` and the non-baseline level `manual`. The slope tells us the changes in `hwy` when *the transmission is changed __from__ `auto` __to__ `manual`*.
...
$$trans = \begin{cases} 1  & \quad \text{if manual transmission} \\ 0  & \quad \text{if automatic transmission} \end{cases}$$
The new dummy variable $trans$ has values 0 or 1 only, and they are representing the two categories `auto` and `manual`. Furthermore, when the dummy variable is used in regression, the baseline level is always assigned value 0.
...
$$y_{j} = \beta_0 + \beta_1x_j + \epsilon_{j}, \quad j = 1, 2, \dots, n, \quad \epsilon_j\sim N(0, \sigma^2),$$ 
where $$x_j = \begin{cases} 1  & \quad \text{if } j \text{th subject is in the non-baseline level} \\ 0  & \quad \text{if } j \text{th subject is in the baseline level}  \end{cases}$$
