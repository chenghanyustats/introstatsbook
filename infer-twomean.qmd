...
knitr::include_graphics("./images/img-infer/drug.jpeg")
```
:::
...
- Step 5-c:  <span style="color:blue"> We reject $H_0$ if $\small t_{test} > t_{\alpha, n-1}$. Since $\small t_{test} = 3.42 > 1.833  = t_{\alpha, n-1}$, we reject $H_0$.</span>
...
## Inferences from Two Independent Samples: Overview
```{r, echo=FALSE, out.width="100%", fig.align='center'}
knitr::include_graphics("./images/img-infer/indept_samples_framework.jpg")
```
...
:::
- Step 4-c:  <span style="color:blue"> $-t_{0.05, 13} = -1.771$. </span>
- Step 5-c:  <span style="color:blue"> We reject $H_0$ if $\small t_{test} < -t_{\alpha, df}$. $\small t_{test} = -1.66 > -1.771 = -t_{\alpha, df}$, we fail to reject $H_0$. </span>
...
## Sampling Distribution of $\overline{X}_1 - \overline{X}_2$ when $\sigma_1 = \sigma_2 = \sigma$
...
unknown | $\large t_{test} = \frac{(\overline{x}_1 - \overline{x}_2) - \color{blue}{D_0}}{\color{red}{s_p}\ \color{black} sqrt{\frac{1}{n_1} + \frac{1}{n_2}}}$ | $\large (\overline{x}_1 - \overline{x}_2) \pm t_{\alpha/2, df} \color{red}{s_p}\ \color{black} sqrt{\frac{1}{n_1} + \frac{1}{n_2}}$
...
- Step 5-c:  <span style="color:blue"> We reject $H_0$ if $\small t_{test} < -t_{\alpha, df}$. Since $\small t_{test} = -7.72 < -1.734  = -t_{\alpha, df}$, we reject $H_0$.</span>
