...
- **Hypothesis testing** <sup>1</sup> is a procedure to **decide whether to reject $H_0$ or not** by how much evidence against $H_0$. 
...
- `r emoji('sunglasses')` No worries, we will learn this step by step!
...
- `r emoji('point_right')` $z_{\alpha}$ is such that $P(Z > z_{\alpha}) = \alpha$ and $Z \sim N(0, 1)$.

- `r emoji('point_right')` $t_{\alpha, n-1}$ is such that $P(T > t_{\alpha, n-1}) = \alpha$ and $T \sim t_{n-1}$.
...
<!-- - $t_{0.025, 20} =$ `r round(qt(0.975, 20), 2)`, $t_{0.05, 20} =$ `r round(qt(0.95, 20), 2)` -->
...
```{r testing_conclusion, echo=FALSE, out.width="100%", fig.align='center', fig.cap="https://www.drdawnwright.com/category/statistics/"}
knitr::include_graphics("./images/img-infer/testing_conclusion.png")
```

```{r yoda, echo=FALSE, out.width="100%", fig.align='center', fig.cap="https://www.pinterest.com/pin/287878601159173631/"}
...
```{r, echo=FALSE, out.width="100%", fig.align='center', fig.cap="https://www.statisticssolutions.com/wp-content/uploads/2017/12/rachnovblog.jpg"}
